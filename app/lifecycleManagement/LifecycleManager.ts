...
export class LifecycleManager {
  ...
  private versionChecker = () => {
    ...
    .then(response => {
      if (response.statusCode === 200) {
        Logger.instance.lcms.info(`Response Recieved, httpStatus: ${response.statusCode}`);
        this.handleVersionInfo(response.content.toString());
      } else {
        const versionResponseText: string = 'LCMS GET Version Response Error Response Status:';
        const bodyText = `Body: ${response.content.toString()}`;
        if (bodyText.indexOf('<code>NotFoundException</code>') > 0) {
          this.setAppUpdateStatus(ActionStatus.Success, 'AppUpdate is not enabled or no revision found from backend');
        } else {
          this.setAppUpdateStatus(ActionStatus.Failure, `${versionResponseText} ${response.statusCode} | ${bodyText}`);
        }
        Logger.instance.lcms.error(`${versionResponseText} ${response.statusCode} | ${bodyText}`);
        this.startVersionCheckerTimer();
      }
    }, (error) => {
  ...
  private appDownloader = (version: LifecycleAppVersionInfo) => {
    ...
    if (this.getStagedVersion() >= version.revision) {
      ...
      .then((response) => {
        Logger.instance.lcms.info(`App download response code: ${response.statusCode}`);

        this.updateApplication(response.content.toFile(LifecycleManager.TEMP_SAVE_PATH), version.revision);
        this.startVersionCheckerTimer();
  ...
  private updateApplication(sourceFile: fs.File, newVersion: number): void {
    ...
    const appExtractWorker = new TsWorker();
