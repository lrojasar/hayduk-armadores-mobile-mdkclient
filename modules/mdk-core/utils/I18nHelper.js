...
var I18nHelper = (function () {
    ...
    I18nHelper._getMDKDefinition = function (fileNameRef) {
        var _this = this;
        var folderPath = this.mdkI18nPath + '/';
        var filePath = folderPath + fileNameRef;
        var currentAppPath = fs.knownFolders.currentApp().path;
        var mdkI18nFilePath = fs.path.join(currentAppPath, filePath);
        if (!fs.File.exists(mdkI18nFilePath)) {
            return null;
        }
        var file = fs.File.fromPath(mdkI18nFilePath);
        var fileData = file.readTextSync(function (e) {
            _this._onParseFail('I18nHelper._getMDKDefinition() - strings file not found: ', fileNameRef, e);
        });
        return fileData;
    };
