            ...
            Object.keys(objectCell).forEach(function (key) {
                if (key !== 'OnPress' && key !== 'OnAccessoryButtonPress' &&
                    ...
                    if (key === 'Icons') {
                        ...
                        promises.push(_this._bindValue(bindingObject, key, objectCell[key], styles && styles.hasOwnProperty(key) ? styles[key] : null).then(function (value) {
                            item[key] = value;
                        }));
                    }
                }
                ...
                if (PropertyTypeChecker_1.PropertyTypeChecker.isDynamicTargetPath(value)
                    ...
                    if (props && props.length > 0) {
                        for (var _b = 0, props_1 = props; _b < props_1.length; _b++) {
                            var prop = props_1[_b];
                            if (searchKeys.indexOf(prop) === -1 && prop.indexOf('/#') === -1 && (isEntitySetResolvable || DataHelper_1.DataHelper.getPropertyType(service, definition.target.EntitySet, prop) === 'string')) {
