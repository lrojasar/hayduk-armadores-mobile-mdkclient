...
var ODataServiceProvider = (function () {
    ...
    ODataServiceProvider.toJSError = function (error) {
        ...
        if (error instanceof Error) {
            ...
            return new Error(messageList[0]);
        }
            ...
            if (_this.dataService != null && _this.isOnline()) {
                try {
                    ...
                    _this.dataService.loadMetadataAsync(new oDataPkg.core.Action0({
                        ...
                        call: function (error) { return reject(ODataServiceProvider.toJSError(error)); },
                    }), _this.getHttpHeaders(params.serviceHeaders));
                }
                catch (error) {
                    reject(ODataServiceProvider.toJSError(error));
                }
                ...
                var _loop_1 = function (mediaContent) {
                    ...
                    for (var key in properties) {
                        if (key) {
                            ...
                            entity.setDataValue(property, converter.convert(properties[key], property.getDataType().getCode(), property.getDataType().getName()));
                        }
                    }
                    try {
            ...
            try {
                ...
                var _loop_2 = function (mediaContent) {
                    ...
                    for (var key in properties) {
                        if (key) {
                            var property = entityType.getProperty(key);
                            entityVal.setDataValue(property, converter.convert(properties[key], property.getDataType().getCode(), property.getDataType().getName()));
                        }
                    }
                    try {
        ...
        return new Promise(function (resolve, reject) {
            try {
                ...
                for (var key in properties) {
                    if (key) {
                        var property = entityType.getProperty(key);
                        entity_2.setDataValue(property, ODataServiceUtils_1.ODataServiceUtils.convert(properties[key], property.getDataType().getCode()));
                    }
                }
                try {
    ...
    ODataServiceProvider.prototype.uploadStream = function (entitySetName, properties, query, readLink, headers, requestOptions) {
        ...
        return new Promise(function (resolve, reject) {
            try {
                if (!_this.isOnline()) {
                    reject(new Error(ErrorMessage_1.ErrorMessage.format(ErrorMessage_1.ErrorMessage.ODATA_INVALID_DATAPROVIDER)));
                }
                var entitySet = _this.dataService.getEntitySet(entitySetName);
                var entityType = entitySet.getEntityType();
                    ...
                    if (methodParameters !== null) {
                        for (var i = 0; i < methodParameters.length(); i++) {
                            ...
                            if (functionParameters != null) {
                                ...
                                inputParam.setValue(converter.convert(parameterValue, parameter.getType().getCode(), parameter.getType().getName()));
