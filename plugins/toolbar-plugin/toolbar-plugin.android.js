...
var ToolBar = (function (_super) {
    ...
    ToolBar.prototype._isItemBeingRendered = function (item) {
        if (item.systemItem) {
            var parsedSystemItem = SystemItem.parse(item.systemItem.toString(), 'black');
    ...
    ToolBar.prototype.getImageOrLabel = function (item, currentItemIndex, rightMostItemIndex) {
        ...
        if (item.systemItem || item.android.systemIcon || item.icon) {
            ...
            if (item.systemItem || item.android.systemIcon) {
                ...
                if (item.systemItem) {
                    parsedSystemItem = SystemItem.parse(item.systemItem.toString(), 'black');
                }
        ...
        if (key.indexOf('_') > 0) {
            ...
            if (this.isValid(key)) {
                ...
                if (value !== '') {
                    ...
                    value = style ? value + '_' + style : value + '_white';
                    value = value + '_24';
                }
                return value;
            }
